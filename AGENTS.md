...
# Documentation
just docs-generate        # Generate docs from source (CLI + API docstrings)
just docs-serve           # Build and serve docs locally with live reload
just docs-build           # Build documentation site
just docs-clean           # Remove generated documentation files
...
- **MkDocs** with **Material for MkDocs** theme for the documentation site
- **mkdocstrings** for automatic API documentation from Python docstrings
- **Typer** CLI docs generation for command-line reference

### Documentation Generation

Documentation is automatically generated from source code:

1. **CLI Documentation**: Generated using Typer's built-in `utils docs` command
2. **API Documentation**: Generated by mkdocstrings from Python docstrings (Google style)

```bash
# Generate CLI documentation only
just docs-generate
...
├── cli/                  # CLI reference
│   ├── index.md
│   └── commands.md       # Auto-generated from Typer
└── api/                  # API reference (uses mkdocstrings)
    ├── index.md
    ├── search.md         # CVESearchService
    ├── downloader.md     # DownloadService
    ├── extractor.md      # ExtractorService
    ├── embeddings.md     # EmbeddingsService
    ├── artifact_fetcher.md
...
When adding or modifying code, write comprehensive Google-style docstrings that mkdocstrings will include in the documentation:
...
def search(
    ...
    Raises:
        FileNotFoundError: If CVE database not found.

    Example:
        >>> search = CVESearchService()
        >>> results = search.search("apache")
        >>> print(results.count)
    """
```
